// Modified version of...
// https://github.com/vocodedev/vocode-react-sdk/blob/main/src/hooks/conversation.ts
//
// Transcripts are being duplicated in the original version.
// There's no error handling in decoding the audio data in the original version
// which causes the audio to stop playing and the whole conversation to stop.
  ...
  const startConversation = async () => {
    ...
    socket.onmessage = (event) => {
      ...
      if (message.type === "websocket_audio") {
        ...
        setTranscripts((tx) => {
          if (tx.length > 0) {
            if (message.text === tx[tx.length - 1].text) {
              return tx;
            }
          }
          tx.push({
